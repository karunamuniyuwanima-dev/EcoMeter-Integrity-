# ============================================================
# detection_engine.py
# EcoMeter Integrity — Anomaly Detection Module  v3
...
def load_data(normal_path, attacked_path):
    ...
    baseline.columns = ['meter_id','hour','base_kwh','base_voltage','base_current','base_pf']

    df_n['reading_kwh']  = df_n['energy_kwh']
    df_n['ground_truth'] = 0
    df_n['attack_type']  = 'none'
    df_a['reading_kwh']  = df_a['attacked_kwh']
    df_a['ground_truth'] = 1

    keep = ['meter_id','timestamp','energy_kwh','reading_kwh',
            'voltage','current','power_factor','ground_truth','attack_type']
    df = pd.concat([
        df_n[[c for c in keep if c in df_n.columns]],
        df_a[[c for c in keep if c in df_a.columns]]
    ]).sort_values(['meter_id','timestamp']).reset_index(drop=True)
    ...
    print("=" * 55)
    print("  ECOMETER INTEGRITY — DETECTION ENGINE v3")
    print("=" * 55)
    print(f"  Total rows      : {len(df):,}")
    print(f"  Normal readings : {(df['ground_truth']==0).sum():,}")
    print(f"  Attack readings : {(df['ground_truth']==1).sum():,}")
...
def engineer_features(df):
    ...
    for meter_id, grp in df.groupby('meter_id'):
        ...
        grp['prev_reading'] = grp['reading_kwh'].shift(1).bfill()
        grp['change_pct']   = (grp['reading_kwh'] - grp['prev_reading']) / (grp['prev_reading'] + 0.001) * 100
        parts.append(grp)
...
def score_row(row):
    ...
    if gap < THRESHOLDS['kwh_volt_gap_low']:
        score += 50; rules.append('energy_drop_gap')
    if row['kwh_pct'] < THRESHOLDS['baseline_drop']:
        score += 30; rules.append('baseline_drop')
    if gap > THRESHOLDS['kwh_volt_gap_high']:
        score += 55; rules.append('energy_spike_gap')
    if row['kwh_pct'] > THRESHOLDS['baseline_spike']:
        score += 40; rules.append('baseline_spike')
    if abs(row['zscore']) > THRESHOLDS['zscore_thresh']:
        score += 20; rules.append('zscore_outlier')
    if row['reading_kwh'] < THRESHOLDS['min_kwh']:
        score += 25; rules.append('below_minimum')
...
def run_detection(df):
    ...
    print(f"  Running detection rules on {len(df):,} readings...")
...
def evaluate_accuracy(df):
    ...
    if fp==0 or fn==0:
        print(f"  Warning: confusion matrix has a zero cell")
    else:
...
def generate_graphs(df, metrics):
    ...
    m1n = df[(df['meter_id']=='M001')&(df['ground_truth']==0)].head(96).reset_index(drop=True)
    ax1.plot(m1n.index, m1n['energy_kwh'],
             color='steelblue', lw=1.5, label='Normal reading', alpha=0.9, zorder=3)
    if 'attack_type' in df.columns:
        for at, col in colors_at.items():
            sub = df[df['attack_type']==at].head(96).reset_index(drop=True)
            if len(sub) > 0:
                ax1.plot(sub.index, sub['reading_kwh'],
                         color=col, lw=1, label=f'{at} attack', alpha=0.6)
    ...
    # Bottom right: detection per attack type
    ...
    if len(at_labels) > 0:
        ...
        for i in range(len(at_labels)):
            ax3.text(i, at_detected[i]+at_missed[i]+20,
                     f'{at_rates[i]:.0f}%', ha='center', fontsize=9, fontweight='bold')
    else:
        ax3.set_title('Detection Rate per Attack Type', fontweight='bold')
        ax3.text(0.5, 0.5, 'No attack type data',
    ...
    # ── GRAPH 3: Risk score + rules triggered ─────────────────
    ...
    if 'rule_triggered' in df.columns:
        safe_mask   = df['rule_triggered'].notna() & (df['rule_triggered'] != 'none')
        rule_counts = df.loc[safe_mask,'rule_triggered'].value_counts().head(8)
    else:
    ...
    if len(rule_counts) > 0:
        ...
        for i, val in enumerate(rule_counts.values):
            ax3b.text(val+20, i, f'{val:,}', va='center', fontsize=10)
    else:
...
def get_summary_for_api(df, metrics):
    ...
    return {
        ...
        'max_risk_score'    : int(df['risk_score'].max()),
