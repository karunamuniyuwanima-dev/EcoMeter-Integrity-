...
THRESHOLDS = {
    ...
    'baseline_drop'    : -25,    # % drop from hourly baseline
    'baseline_spike'   :  50,    # % spike from hourly baseline
    'zscore_thresh'    :  1.3,   # noise: statistical outlier
...
def engineer_features(df):
    ...
    for meter_id, grp in df.groupby('meter_id'):
        ...
        # Rolling stats (12 readings = 4 hours)
        grp['roll_mean'] = grp['reading_kwh'].rolling(12, min_periods=4).mean()
        grp['roll_std']  = grp['reading_kwh'].rolling(12, min_periods=4).std().fillna(0.001)
...
def generate_graphs(df, metrics):
    ...
    axes2[0].axhline(90, color='green', linestyle='--', alpha=0.6, label='90% target')
    axes2[0].legend()
